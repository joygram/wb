<?php
	...
	switch($part)
	{
		...
		case "category":
			...
			break ;
		case "pagebar":
			$target_dir = "$C_base[dir]/board/skin/__global/pagebar" ;
			$title = _L_PAGEBAR ;
			break ;
		default:
			$target_dir = "$C_base[dir]/board/skin" ;
			$title = _L_BOARD ;
			break ;
	}
	...
	while( ($file_name = $flist->next()) )
	{
		...
		*/
		...
		if (empty($part)) 
		{
			$_skin_dir = "$C_base[dir]/board/skin/$file_name" ;
			$_skin_url = "$C_base[url]/board/skin/$file_name" ;
		}
		else
		{
			$_skin_dir = "$C_base[dir]/board/skin/__global/$part/$file_name" ;
			$_skin_url = "$C_base[url]/board/skin/__global/$part/$file_name" ;
		}
		$i++ ;
		...
		//$CONFIG_URL = "javascript:onClick=POP(\"board_open_config.php?conf=$file_name\"); " ; 
